...
// Синглтон-предохранитель EsiShield остается без изменений
const EsiShield = {
...
/**
 * Универсальный пакетный обработчик запросов.
 * Гарантирует сохранение исходного порядка и длины массива.
 * Вместо удаления сбойных элементов возвращает null, передавая контроль вызывающему коду.
 */
export async function queryHandler<T>(urls: string[]): Promise<(T | null)[]> {
    ...
    // Возвращаем полный массив результатов "как есть", сохраняя позиции элементов
